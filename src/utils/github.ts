...
export const uploadFilesBatch = async (
  ...
  const { data: newTree } = await octokit.git.createTree({
    ...
    repo: repo.name,
    tree: treeData,
    ...(treeSha && treeSha !== '4b825dc642cb6eb9a060e54bf8d69288fbee4904' ? { base_tree: treeSha } : {}),
  });
    ...
    const { data: newCommit } = await octokit.git.createCommit({
      ...
      message,
      tree: newTree.sha,
      parents: commitSha && commitSha !== '4b825dc642cb6eb9a060e54bf8d69288fbee4904' ? [commitSha] : [],
    });
...
export const deleteFile = async (
  ...
  try {
    ...
    if (error.status === 404) {
      throw new Error('파일을 찾을 수 없습니다. 이미 삭제되었을 수 있습니다.');
    }
    throw error;
  }
};
