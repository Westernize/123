...
const FileBrowser: React.FC<FileBrowserProps> = ({ octokit, repo, onFileDeleted, onFileUpdated }) => {
  ...
  const handleDelete = async (file: RepositoryFile) => {
    ...
    const newFiles = previousFiles.filter((f) => f.path !== file.path);
    setFiles(newFiles);

    // 폴더 내 모든 파일이 삭제되면 상위 폴더로 이동
    if (newFiles.length === 0 && currentPathRef.current !== '') {
      setTimeout(() => {
      ...
      {editFile && (
        <FileEditor
          ...
          onDelete={() => {
            ...
            const newFiles = currentFiles.filter((f) => f.path !== pathToDelete);
            setFiles(newFiles);
