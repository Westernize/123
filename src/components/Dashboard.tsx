...
const Dashboard: React.FC = () => {
  ...
  const handleFileUpload = async (files: File[], commitMessage?: string) => {
    ...
    try {
      ...
      // 진행률 초기화
      const progress: UploadProgress[] = fileUploads.map((file) => ({
      ...
      // 진행률 업데이트 함수
      const updateProgress = (index: number, status: UploadProgress['status'], progressValue?: number, error?: string) => {
        setUploadProgress((prev) => {
          const newProgress = [...prev];
          newProgress[index] = {
            ...newProgress[index],
            status,
            progress: progressValue !== undefined ? progressValue : (status === 'uploading' ? 50 : status === 'success' ? 100 : 0),
            error,
          };
          return newProgress;
        });
      ...
      // 순차적으로 파일 업로드
      const message = commitMessage || `Upload ${fileUploads.length} file(s) via GitHub Portfolio Uploader`;
      let successCount = 0;
      let errorCount = 0;
      ...
      for (let i = 0; i < fileUploads.length; i++) {
        ...
        try {
          ...
          await uploadFile(
            ...
            selectedRepo,
            fileUpload,
            i === 0 ? message : undefined // 첫 번째 파일에만 커밋 메시지 포함
          );

          // 업로드 완료
          updateProgress(i, 'success', 100);
          successCount++;
        } catch (error: any) {
          // 업로드 실패
          updateProgress(i, 'error', 0, error.message || '업로드 실패');
          errorCount++;
        }
      }
      ...
      if (errorCount === 0) {
        setUploadStatus({
          success: true,
          message: `${successCount}개의 파일이 성공적으로 업로드되었습니다!`,
        });
      } else {
        setUploadStatus({
          success: false,
          message: `${successCount}개 성공, ${errorCount}개 실패`,
        });
      }
