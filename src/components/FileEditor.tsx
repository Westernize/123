...
import { getFileContent, uploadFile, deleteFile } from '../utils/github';
...
const FileEditor: React.FC<FileEditorProps> = ({ file, octokit, repo, onClose, onSave, onDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // 404 에러는 이미 삭제된 것이므로 성공 처리
      if (err.status === 404 || err.message?.includes('Not Found') || err.message?.includes('이미 존재하지 않습니다')) {
        onClose();
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-[#161b22] rounded-xl border border-[#30363d] w-full max-w-[95vw] h-[95vh] max-h-[95vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[#30363d]">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              disabled={saving || deleting}
              className="flex items-center gap-2 px-4 py-2 bg-[#da3633] text-white rounded-lg hover:bg-[#f85149] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {deleting ? (
                <Loader2 className="w-4 h-4 animate-spin" />
              ) : (
        ...
        <div className="flex-1 overflow-hidden p-4 min-h-0">
          <textarea
            ...
            className="w-full h-full min-h-[500px] p-4 bg-[#0d1117] border border-[#30363d] rounded-lg text-[#c9d1d9] font-mono text-sm resize-none focus:ring-2 focus:ring-[#1f6feb] focus:border-[#1f6feb] outline-none"
