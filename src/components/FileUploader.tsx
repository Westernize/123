...
import { Upload, File, X, Loader2, Folder, Sparkles } from 'lucide-react';
...
interface FileUploaderProps {
  ...
  onTemplateSelect?: (files: FileUpload[]) => void;
...
const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const handleFileSelect = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      // 폴더 선택인 경우 webkitRelativePath가 설정되어 있음
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex items-center gap-2 justify-center flex-wrap">
          ...
          {onTemplateSelect && onToggleTemplates && (
            <button
              onClick={onToggleTemplates}
              className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-colors font-medium shadow-md ${
                showTemplates
                ...
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-[#8b949e]">
                      {(file as any).webkitRelativePath ? (file as any).webkitRelativePath : formatFileSize(file.size)}
                    </p>
                  </div>
                </div>
                {!uploading && (
      ...
      {/* Template Selector */}
      {showTemplates && onTemplateSelect && (
        <div className="border border-[#30363d] rounded-xl p-4 bg-[#0d1117] relative">
